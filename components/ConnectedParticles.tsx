...
export default function ConnectedParticles({ className = "", count = 48, hue = 200 }: ConnectedParticlesProps) {
  ...
  useEffect(() => {
    ...
    const maxDist = Math.min(160, Math.max(100, Math.min(w, h) * 0.25));
  ...
  c.fillRect(0, 0, w, h);
      ...
      for (const n of nodes) {
        ...
        n.a += 0.002 + n.z * 0.004;
        // gentle flow
        n.vx += Math.cos(n.a) * 0.01 * (0.5 + n.z);
        n.vy += Math.sin(n.a * 1.2) * 0.01 * (0.5 + n.z);
        // bounds wrap
      ...
      for (let i = 0; i < nodes.length; i++) {
        for (let j = i + 1; j < nodes.length; j++) {
          ...
          if (d < maxDist) {
            ...
            const alpha = 0.08 + t * 0.25 + depth * 0.1;
            c.strokeStyle = `hsla(${hue + depth * 40}, 70%, ${45 + depth * 35}%, ${alpha})`;
            ...
            if (t > 0.75 && Math.random() < 0.02) {
              ...
              c.lineTo(k.x, k.y);
              c.closePath();
              c.strokeStyle = `hsla(${hue + depth * 60}, 80%, ${55 + depth * 25}%, ${alpha * 0.6})`;
              c.lineWidth = 0.5;
              c.stroke();
            }
          }
      ...
      for (const n of nodes) {
        const r = 1.2 + n.z * 2.2;
        c.fillStyle = `hsla(${hue + n.z * 50}, 80%, ${60 + n.z * 25}%, ${0.4 + n.z * 0.4})`;
        c.beginPath();
